...
/**
 * $Id: dvdedf.cpp 1360 2016-02-19 09:34:18Z klugeflo $
 * @file dvdedf.cpp
...
namespace tmssim {
  ...
  double DVDEDFScheduler::calcValue(TmsTime time, const Job *job) const {
    return job->getPossibleExecValue(time) / job->getRemainingExecutionTime();
  }


  Scheduler* DVDEDFSchedulerAllocator() { return new DVDEDFScheduler; }
